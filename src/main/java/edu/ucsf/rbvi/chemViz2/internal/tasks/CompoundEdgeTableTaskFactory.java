...
public class CompoundEdgeTableTaskFactory extends ChemVizAbstractTaskFactory 
                                          implements NetworkTaskFactory, EdgeViewTaskFactory {
	ChemInfoSettings settings = null;
	Scope scope;

	public CompoundEdgeTableTaskFactory(ChemInfoSettings settings, Scope scope) {
	...
	public TaskIterator createTaskIterator(View<CyEdge> eView, CyNetworkView netView) {
		return new TaskIterator(new CompoundTableTask(netView.getModel(), eView.getModel(), scope, settings));
	}
