...
public class UpdateCompoundsTask extends AbstractCompoundTask {
	...
	/**
 	 ...
 	 */
  public UpdateCompoundsTask(CyNetwork network, List<CyIdentifiable> objectList, ChemInfoSettings settings) {
		super(settings);
		this.network = network;
		this.nodeList = new ArrayList<CyIdentifiable>();
		this.edgeList = new ArrayList<CyIdentifiable>();
		for (CyIdentifiable id: objectList) {
	...
	public void run(TaskMonitor taskMonitor) {
		getCompounds(nodeList, network,
 	               settings.getCompoundAttributes("node",AttriType.smiles),
 	               settings.getCompoundAttributes("node",AttriType.inchi),
	               settings.getMaxThreads());

		if (edgeList == null || edgeList.size() == 0) {
			getCompounds(edgeList, network,
 		               settings.getCompoundAttributes("edge",AttriType.smiles),
 		               settings.getCompoundAttributes("edge",AttriType.inchi),
		               settings.getMaxThreads());
		}
	}
