...
public class CompoundNodePopupTaskFactory extends ChemVizAbstractTaskFactory 
                                          implements NetworkTaskFactory, NodeViewTaskFactory {
	ChemInfoSettings settings = null;
	Scope scope;

	public CompoundNodePopupTaskFactory(ChemInfoSettings settings, Scope scope) {
	...
	public TaskIterator createTaskIterator(CyNetwork network) {
		return new TaskIterator(new CompoundPopupTask(network, null, null, scope, settings));
	}

	public TaskIterator createTaskIterator(View<CyNode> nView, CyNetworkView netView) {
		return new TaskIterator(new CompoundPopupTask(netView.getModel(), nView.getModel(), null, scope, settings));
	}
