...
public class CreateNodeAttributesTaskFactory extends ChemVizAbstractTaskFactory 
                                             implements NetworkTaskFactory, NodeViewTaskFactory {
	ChemInfoSettings settings = null;
	Scope scope;

	public CreateNodeAttributesTaskFactory(ChemInfoSettings settings, Scope scope) {
	...
	public TaskIterator createTaskIterator(View<CyNode> nView, CyNetworkView netView) {
		return new TaskIterator(new CreateAttributesTask(netView.getModel(), nView.getModel(), scope, settings));
	}
