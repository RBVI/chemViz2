...
public class CompoundEdgePopupTaskFactory extends ChemVizAbstractTaskFactory 
                                          implements NetworkTaskFactory, EdgeViewTaskFactory {
	ChemInfoSettings settings = null;
	Scope scope;

	public CompoundEdgePopupTaskFactory(ChemInfoSettings settings, Scope scope) {
	...
	public TaskIterator createTaskIterator(View<CyEdge> eView, CyNetworkView netView) {
		return new TaskIterator(new CompoundPopupTask(netView.getModel(), eView.getModel(), null, scope, settings));
	}
