...
final class ReactionDepiction extends Depiction {
    ...
    @Override
    public void toShapes(IDrawVisitor shapeVisitor) {
        // format margins and padding for raster images
        final double scale = model.get(BasicSceneGenerator.Scale.class);
        final double zoom = model.get(BasicSceneGenerator.ZoomFactor.class);
        final double margin = getMarginValue(DepictionGenerator.DEFAULT_PX_MARGIN);
        ...
        shapeVisitor.setTransform(AffineTransform.getScaleInstance(1,-1));
        shapeVisitor.visit(new RectangleElement(0, -(int) Math.ceil(total.h), (int) Math.ceil(total.w), (int) Math.ceil(total.h),
                                           true, model.get(BasicSceneGenerator.BackgroundColor.class)));
        ...
        for (int i = 0; i < mainComp.size(); i++) {
            ...
            if (i == arrowIdx) {
                w = rescale * (xOffsets[i + 1] - xOffsets[i]) + (nSideCol - 1) * padding;
                draw(shapeVisitor, 1, createArrow(w, arrowHeight * rescale), rect(x, y, w, h));
                continue;
            }
            ...
            draw(shapeVisitor, zoom, bounds, rect(x, y, w, h));
        }
        ...
        if (!title.isEmpty()) {
            double y = yBase + nRow * padding + rescale * yOffsets[nRow];
            double h = rescale * title.height();
            draw(shapeVisitor, zoom, title, rect(0, y, total.w, h));
        }
        ...
        for (int i = 0; i < sideComps.size(); i++) {
            ...
            draw(shapeVisitor, zoom, sideComps.get(i), rect(x, y, w, h));
        }
